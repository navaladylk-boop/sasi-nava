...
=================== HIKVISION USER SEARCH QUERY START ===================`);
    ...
    while (hasMore) {
      ...
      try {
        ...
        const userList = Array.isArray(matches) ? matches : [matches];
        ...
        if (userList.length > 0) {
          ...
          userList.forEach((u) => {
            ...
            if (empNo) {
              ...
              const cardNo = u.cardNo ? String(u.cardNo).trim() : Array.isArray(u.cards) && u.cards[0] ? String(u.cards[0]).trim() : void 0;
              const numOfCard = Number(u.numOfCard || 0);
              const numOfFP = Number(u.numOfFP || 0);
              const numOfFace = Number(u.numOfFace || 0);
              const userVerifyMode = u.userVerifyMode ? String(u.userVerifyMode) : void 0;
              const doorRight = u.doorRight ? String(u.doorRight) : void 0;
          ...
          if (empNo) {
            ...
            if (!usersMap.has(empNo)) {
              usersMap.set(empNo, {
                ...
                userType,
                gender: gender ? gender.trim() : void 0,
                cardNo: cardNo ? cardNo.trim() : void 0,
                numOfCard,
                numOfFP,
                numOfFace
              });
...
import_electron2.ipcMain.handle("device:hikvision-search-users", async (event, config) => {
  try {
    ...
    return {
      ...
      count: 0,
      isUnsupported,
      message: isUnsupported ? "Hikvision user synchronization is not supported by this device/API." : `Failed to retrieve Hikvision users: ${err.message}`
    };
  }
});
